...
TEST(Matrix, Addition) {
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        ASSERT_TRUE(
                isAlmostEqual(m1 + m2,
                        Matrix<double>(
                                m1.toEigenMatrix() + m2.toEigenMatrix())));
    }
}
...
TEST(Matrix, Multiplication) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        ASSERT_TRUE(
                isAlmostEqual(m1 * m2,
                        Matrix<double>(
                                (m1.toEigenMatrix() * m2.toEigenMatrix()).eval())));
    }
}
...
TEST(Matrix, MultiplicationStrassen) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        m2.random(g);
        ASSERT_TRUE(
                isAlmostEqual(strassen<1>(m1, m2),
                        Matrix<double>(
                                (m1.toEigenMatrix() * m2.toEigenMatrix()).eval())));
    }
}
...
TEST(Matrix, EigenMap) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        ASSERT_TRUE(
                isAlmostEqual(Matrix<double>(map1 * map2),
                        Matrix<double>(
                                (m1.toEigenMatrix() * m2.toEigenMatrix()).eval())));
    }
}
...
TEST(Matrix, MultipleOperations) {
    ...
    for (int i = 0; i < 20; ++i) {
        m1.random(g);
        m2.random(g);
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> m1e =
                m1.toEigenMatrix();
        Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> m2e =
                m2.toEigenMatrix();
        ASSERT_TRUE(
                isAlmostEqual(-(m1 + m1) * m2 + m2 - m2 + m2 - m2,
                        Matrix<double>(
                                -(m1e + m1e) * m2e + m2e - m2e + m2e - m2e)));
    }
}
