...
//std::enable_if_t<vectorizable_v<E>> subtraction_evaluate(const MatrixExpression<E>& expr, RefSubMatrix<scalar_type_t<E>, C> dst) {
//    using T = scalar_type_t<E>;
//    using PacketScalar = typename Vc::native_simd<T>;
//
//    const int packet_size = PacketScalar::size();
...
//        point_type p{coord.x, j};
//
//        (dst.template packet<PacketScalar, Vc::flags::element_aligned_tag>(p) - expr.template packet<PacketScalar, Vc::flags::element_aligned_tag>(p)).copy_to(&dst[p], Vc::flags::element_aligned);
//    });
//
//    for(int i = 0; i < expr.rows(); ++i) {
//        for(int j = caligned_end; j < expr.columns(); ++j) {
